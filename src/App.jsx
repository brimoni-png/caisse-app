...
function Reports({ txs, members, lang, xlsxReady, chartReady, onRefresh, onReset, onAddTx, onUpdateTx, onAddMember }) {
  ...
  function doExport(mode) {
    ...
    const txAoa = [
      ...
      // Row 4 – headers
      ["#","Date","Type","Membre / Payeur","Montant (MRU)","Description / Note","Mois","Année","Statut","Réf.","ID_SYSTEM"],
      // Data rows
    ...
    wsT["!cols"] = [{wch:5},{wch:14},{wch:16},{wch:22},{wch:18},{wch:32},{wch:10},{wch:9},{wch:13},{wch:14},{wch:0,hidden:true}];

    // Merges: banner rows span A:K (cols 0-10)
    wsT["!merges"] = [
    ...
    // Style header row 4
    const txHdrs = ["A","B","C","D","E","F","G","H","I","J","K"];
    txHdrs.forEach(col => {
      const c = wsT[`${col}4`];
  ...
  return (
    <div style={{ direction: t.dir, padding: "10px 0" }}>
      ...
      <div style={{ marginTop: 6, borderTop: `1px solid ${C.outline}`, paddingTop: 20 }}>
        ...
        <input ref={importRef} type="file" accept=".xlsx,.xls" style={{ display: "none" }} onChange={e => { const f = e.target.files?.[0]; if (f) doImport(f); }} />
        <button className="tbtn eco-btn" onClick={() => { if (xlsxReady && !importing) importRef.current?.click(); }} disabled={!xlsxReady || importing}
