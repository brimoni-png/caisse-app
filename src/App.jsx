...
function Settings({ lang, setLang, t, onLogout, readOnly = false }) {
  ...
  const [pinStep, setPinStep] = useState("current"); // "current" | "new" | "confirm" | "success"
  ...
  const triggerShake = (msg) => {
    setPinError(msg);
    setPinShake(true);
    setTimeout(() => { setPinShake(false); setPinError(""); }, 1600);
  };

  const handlePinModal = () => {
    setPinModal(true); setPinStep("current");
    setPinCurrent(""); setPinNew(""); setPinConfirm(""); setPinError("");
  };
  const closePinModal = () => { setPinModal(false); setPinStep("current"); setPinError(""); };

  const handlePinNext = () => {
    if (pinStep === "current") {
      const expected = readOnly ? getPinReadonly() : getPin();
      if (pinCurrent !== expected) return triggerShake(isAr ? "رمز PIN الحالي غير صحيح" : "PIN actuel incorrect");
      setPinStep("new"); setPinCurrent("");
    } else if (pinStep === "new") {
      if (pinNew.length < 4) return triggerShake(isAr ? "الرمز يجب أن يكون 4 أرقام على الأقل" : "Le PIN doit contenir au moins 4 chiffres");
      setPinStep("confirm"); setPinNew(pinNew);
    } else if (pinStep === "confirm") {
      if (pinConfirm !== pinNew) return triggerShake(isAr ? "رمزا PIN غير متطابقان" : "Les PIN ne correspondent pas");
      if (readOnly) savePinReadonly(pinNew); else savePin(pinNew);
      setPinStep("success");
      setTimeout(() => closePinModal(), 1800);
    }
  };

  const pinInpStyle = { width: "100%", background: C.bgLow, border: `1.5px solid ${pinShake ? C.red : "transparent"}`, borderRadius: 14, padding: "13px 16px", fontSize: 20, letterSpacing: 10, color: C.text, outline: "none", fontFamily: "inherit", textAlign: "center", transition: "border-color .2s", animation: pinShake ? "shake .4s ease" : "none" };

  const PinField = ({ value, onChange, placeholder }) => (
    <input type="password" inputMode="numeric" maxLength={8} value={value}
      onChange={e => { onChange(e.target.value.replace(/\D/g,"")); setPinError(""); }}
      onKeyDown={e => e.key === "Enter" && handlePinNext()}
      placeholder={placeholder || "••••"} style={pinInpStyle} autoFocus />
  );
  ...
  const groups = [
    ...
    { title: t.secLbl, items: [{ icon: Ic.lock(C.gold), label: t.changePin, value: "****", action: handlePinModal }] },
  ...
  return (
    <div style={{ direction: t.dir, padding: "10px 0" }}>
      ...
      {/* ── PIN Change Modal ── */}
      {pinModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(1,20,14,0.65)", backdropFilter: "blur(6px)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}
          onClick={e => e.target === e.currentTarget && closePinModal()}>
          <div style={{ background: C.card, borderRadius: "28px 28px 0 0", padding: "28px 24px 36px", width: "100%", maxWidth: 430, boxShadow: "0 -8px 40px rgba(1,45,29,0.18)", animation: "pop .22s ease both" }} dir={t.dir}>
            {/* Header */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, flexDirection: t.dir === "rtl" ? "row-reverse" : "row" }}>
              <div>
                <div style={{ fontWeight: 800, fontSize: 16, color: C.text }}>{t.changePin}</div>
                <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
                  {pinStep === "current"  && (isAr ? "أدخل رمز PIN الحالي" : "Entrez votre PIN actuel")}
                  {pinStep === "new"      && (isAr ? "أدخل رمز PIN الجديد" : "Entrez votre nouveau PIN")}
                  {pinStep === "confirm"  && (isAr ? "أكّد رمز PIN الجديد" : "Confirmez le nouveau PIN")}
                  {pinStep === "success"  && (isAr ? "تم تغيير الرمز بنجاح ✓" : "PIN modifié avec succès ✓")}
                </div>
              </div>
              <button className="tbtn" onClick={closePinModal} style={{ background: C.bgLow, border: "none", borderRadius: 10, width: 34, height: 34, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
            ...
            {pinStep !== "success" && (
              <div style={{ display: "flex", gap: 6, marginBottom: 22, justifyContent: "center" }}>
                {["current","new","confirm"].map((s, i) => (
                  <div key={s} style={{ height: 4, borderRadius: 4, flex: 1, background: ["current","new","confirm"].indexOf(pinStep) >= i ? C.primaryLt : C.bgLow, transition: "background .3s" }} />
                ))}
              </div>
            )}
            ...
            {pinStep === "success" ? (
              <div style={{ textAlign: "center", padding: "24px 0" }}>
                <div style={{ width: 64, height: 64, borderRadius: "50%", background: "rgba(45,106,79,0.12)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>
                  <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke={C.primaryLt} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <div style={{ fontSize: 15, fontWeight: 700, color: C.primaryLt }}>{isAr ? "تم الحفظ!" : "PIN sauvegardé !"}</div>
              </div>
            ) : (
              <>
                {pinStep === "current"  && <PinField value={pinCurrent}  onChange={setPinCurrent}  placeholder={isAr ? "رمز PIN الحالي" : "PIN actuel"} />}
                {pinStep === "new"      && <PinField value={pinNew}      onChange={setPinNew}      placeholder={isAr ? "رمز PIN جديد" : "Nouveau PIN"} />}
                {pinStep === "confirm"  && <PinField value={pinConfirm}  onChange={setPinConfirm}  placeholder={isAr ? "تأكيد الرمز" : "Confirmer"} />}

                {pinError && (
                  <div style={{ color: C.red, fontSize: 12, fontWeight: 600, textAlign: "center", marginTop: 10, animation: "shake .4s ease" }}>{pinError}</div>
                )}

                <button className="tbtn" onClick={handlePinNext}
                  style={{ width: "100%", marginTop: 20, background: C.heroGrad, border: "none", borderRadius: 16, padding: "14px", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", boxShadow: "0 6px 20px rgba(1,45,29,0.22)" }}>
                  {pinStep === "confirm" ? (isAr ? "حفظ" : "Enregistrer") : (isAr ? "التالي" : "Suivant")}
                </button>
              </>
            )}
          </div>
...
// ─── LOGIN SCREEN ─────────────────────────────────────────────────────────────
const VALID_PIN_DEFAULT          = "2526";
const VALID_USERNAME             = "cheikh";
const VALID_PIN_READONLY_DEFAULT = "2233";
const VALID_USER_READONLY        = "admin";

function getPin()         { try { return localStorage.getItem("cc_pin_main") || VALID_PIN_DEFAULT; } catch { return VALID_PIN_DEFAULT; } }
function getPinReadonly() { try { return localStorage.getItem("cc_pin_ro")   || VALID_PIN_READONLY_DEFAULT; } catch { return VALID_PIN_READONLY_DEFAULT; } }
function savePin(v)        { try { localStorage.setItem("cc_pin_main", v); } catch {} }
function savePinReadonly(v){ try { localStorage.setItem("cc_pin_ro", v); }   catch {} }
...
function LoginScreen({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    // Full access
    if (nameLower === VALID_USERNAME && pin === getPin()) {
      setLoading(true);
    ...
    // Read-only access
    if (nameLower === VALID_USER_READONLY && pin === getPinReadonly()) {
      setLoading(true);
      await new Promise(r => setTimeout(r, 600));
